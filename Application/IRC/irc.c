...
{
	INDEX = 0;										
	EPCON = EPPL(1) | P00_INDEX;	//设置P00为INT2的中断引脚，下降沿中断
	GPIO_Init(P00F,INPUT);				//设置P00为输入引脚
	INT2EN = 1; 									//INT2中断使能
	EPIE |= BIT0;	
	EXIP |= BIT0;									//设置INT2为高优先级中断
}
...
void INT2_ISR (void) interrupt 7
{
	...
	if(EPIF & BIT0)
	{
		EPIF = BIT0;
		TR1=0;
		CrtTimVal=((unsigned int)TH1)<<8|(unsigned int)TL1;			//获取当前TIMER1计数值
		TR1 = 1;		

		if(IRC_JudgeValidPulseTimer)
		{
			PulseWidth = CrtTimVal - LastTimVal;			//减去上次计数值， 得到脉宽
...
unsigned int IRC_GetMsg(void)
{
	...
	if(!IRC_MonitorFlag)
	{
		if(IR_RxEndFlag)
		{
			...
			{
				Msg = (unsigned int)IRC_GetKeyCode(IR_Code[2]);		//得到遥控按键按下的信息
				KeyBak = Msg;						
				IRC_MonitorFlag = 1;
				IRC_LongStartFlag = 0;
	...
	else
	{
		if(!IRC_BreakTimer)
		{
			...
			{
				Msg = KeyBak | KEY_BREAK;		//得到遥控按键短按释放信息
			}
			else
			{
				Msg = KeyBak | KEY_LONG_BREAK;	//得到遥控按键长按释放信息
			}
			IRC_MonitorFlag = 0;			
		}
		else
		{
			if(!IRC_LongKeyTimer)
			{
				...
				{
					IRC_LongStartFlag = 1;
					Msg = KeyBak | KEY_LONG_START;			//得到遥控按键长按开始的信息， 按键按下后大概1秒后产生长按信息
				}
				else
				{
					Msg = KeyBak | KEY_LONG;		//得到长按信息， 长按开始后，大概每300ms产生一次长按信息
				}
